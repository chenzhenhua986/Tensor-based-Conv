...
from ignite.metrics import Accuracy, Loss, Precision, Recall, mIoU, IoU, DiceCoefficient, ConfusionMatrix
    ...
    if idist.get_rank() == 0:
        ...
        try:
            clearml_logger.report_confusion_matrix(
                ...
                matrix=cm,
                iteration=iteration,
                xlabels=data.CityscapesOpencv.target_names,
                ylabels=data.CityscapesOpencv.target_names,
                extra_layout=None,
            )
        except NameError:
...
def setup_experiment_tracking(config, with_clearml, task_type="training"):
    ...
    if idist.get_rank() == 0:
        if with_clearml:
            ...
            task = Task.init("CityScapes Training", config.config_filepath.stem, task_type=task_type)
...
def run_training(config_filepath, backend="nccl", with_clearml=False):
    ...
    with idist.Parallel(backend=backend) as parallel:
        logger = setup_logger(name="CityScapes Training", distributed_rank=idist.get_rank())
...
def run_evaluation(config_filepath, backend="nccl", with_clearml=True):
    ...
    with idist.Parallel(backend=backend) as parallel:
        logger = setup_logger(name="CityScapes Evaluation", distributed_rank=idist.get_rank())
