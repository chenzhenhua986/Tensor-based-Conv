...
class Net(nn.Module):
    def __init__(self, device):
        ...
        self.model = tconv_fcn2(device, 23) 
...
preprocess_transform_pretrain = T.Compose([
                T.Resize(80), # Resize images to 256 x 256
                T.RandomCrop(64), # Center crop image
...
trainset = datasets.VOCSegmentation(root='./data', year='2012', image_set='train', download=False, transform=preprocess_transform_pretrain)
train_loader_pretrain = torch.utils.data.DataLoader(trainset, batch_size=batch_size, shuffle=True, num_workers=2)
testset = datasets.VOCSegmentation(root='./data', year='2012', image_set='val', download=False, transform=preprocess_transform_test)
